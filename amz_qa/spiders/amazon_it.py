...
class AMZITQAInfo(object):
    ...
    def get_qa_ls(self):
        ...
        for item in ls:
            ...
            reg_ret = re.search(r"Da\s+(.+?)\s+in data\s+(.+)$", tmp)
            if reg_ret:
                qa_info['author'] = reg_ret.group(1).strip()
                date = self.month_translate(reg_ret.group(2).strip().lower())
                qa_info['date'] = time.strftime("%Y-%m-%d", time.strptime(date, "%d %B %Y"))
