...
class AMZJPReviewInfo(object):
    ...
    def get_review_ls(self):
        ...
        for item in ls:
            ...
            if len(tmp_ls)>=2:
                review_info['rating'] = float(tmp_ls[1])
            ...
            if tmp_ls:
                date = tmp_ls[0].replace('on', '').strip()
                review_info['date'] = time.strftime("%Y-%m-%d", time.strptime(date, "%Y年%m月%d日"))
